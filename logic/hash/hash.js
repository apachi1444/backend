...
const isInDatabase=async({ email, username, phone })=>{
    ...
    try {
        ...
        if(nameExist) return "This username has been used on Colocakesh, please choose another one";
        const phoneExist=await User.findOne({phone: phone});
        if(phoneExist) return "This phone number has been used on Colocakesh, please choose another one";
        return response;
    } catch (error) {
